...
export const claudePin = "claude-sonnet-4-20250514";
export const gptPin = "gpt-4.1-2025-04-14";

export async function callFillHints(
    model: string,
    filePath: string,
    newFileName: string,
) {
    ...
    try {
        ...
        if (apiKey && dafnyPath) {
            ...
            vscode.window.showErrorMessage(
                "OpenAI API key & Dafny path are required to use the extension.",
            );
        }
    } catch (err) {
...
async function GPTFillHints(
    ...
    vscode.window.showInformationMessage(
        "Generating Dafny hints from the file content...",
    );
...
async function ClaudeFillHints(
...
async function GeminiFillHints(
    ...
    try {
        ...
        if (apiKey && dafnyPath) {
            ...
            if (model.startsWith("gpt")) {
                ...
                translatedCode = await ClaudeTranslatePython(
                    apiKey,
                    pythonCode,
                );
            }
            if (typeof translatedCode === "string") {
                const dafnyFilePath = filePath.replace(
                    /\.py$/,
                    "_translated.dfy",
                );
                await fs.writeFile(dafnyFilePath, translatedCode);
                await vscode.window.showTextDocument(
                    vscode.Uri.file(dafnyFilePath),
                );
...
async function GPTTranslatePython(
    GPTApiKey: string,
    pythonCode: string,
): Promise<string | undefined> {
    const openai = new OpenAI({ apiKey: GPTApiKey });
    vscode.window.showInformationMessage(
        "Translating Python code to Dafny & verifying...",
    );
...
async function ClaudeTranslatePython(
    ClaudeApiKey: string,
    pythonCode: string,
): Promise<string | undefined> {
    const anthropic = new Anthropic({ apiKey: ClaudeApiKey });
...
export async function askGeminiApiKey(): Promise<string> {
    ...
    if (apiKey) {
        await vscode.workspace
            .getConfiguration()
            .update("google.apiKey", apiKey, vscode.ConfigurationTarget.Global);
        return apiKey;
    } else {
        throw new Error("API key is required to use the extension.");
    }
...
export async function ensureAWSAccessKeyId(): Promise<string | undefined> {
    let accessKeyId = vscode.workspace
        .getConfiguration()
...
export async function ensureAWSSecretAccessKey(): Promise<string | undefined> {
    let secretAccessKey = vscode.workspace
        .getConfiguration()
...
export async function askAWSSecretAccessKey(): Promise<string> {
    ...
    if (secretAccessKey) {
        ...
        throw new Error(
            "AWS Secret Access Key is required to use the extension.",
        );
    }
}

export async function readAWSCredentials(): Promise<{
    accessKeyId?: string;
...
export async function ensureAWSCredentials(): Promise<{
    accessKeyId: string;
    secretAccessKey: string;
}> {
...
export async function ensureNumIterations(): Promise<number | undefined> {
    let numIterations = vscode.workspace
        .getConfiguration("dafny-autopilot")
        .get<number>("numIterations");
    if (
        !(numIterations && Number.isInteger(numIterations) && numIterations > 0)
    ) {
...
export async function askNumIterations(): Promise<number> {
    let numIterationsString = await vscode.window.showInputBox({
        ...
        password: false,
        placeHolder:
            "Enter your desired max # of iterations performed by LLM (a positive int)",
    });
    if (!numIterationsString) {
        throw new Error(
            "A positive integer number of iterations must be given to use the extension.",
        );
    } else {
...
export async function callLangChain(
    ...
    try {
        ...
        let dafnyPath = vscode.workspace
            .getConfiguration("dafny-autopilot")
            .get<string>("dafnyPath");
        if (!dafnyPath) {
        ...
        vscode.window.showInformationMessage(
            "Filling in annotations & verifying...",
        );
        ...
        vscode.window.showErrorMessage(
            `Error processing Dafny file: ${(error as Error).message}`,
        );
    } finally {
...
export async function explainDafnyAnnotation(
    model: string,
    selectedText: string,
    outputChannel: vscode.OutputChannel,
) {
    ...
    try {
        ...
        if (model.startsWith("gpt")) {
            explanation = await explainUsingGPT(
                apiKey,
                sysPrompt,
                userPrompt,
                model,
            ...
            explanation = await explainUsingClaude(
                apiKey,
                sysPrompt,
                userPrompt,
                model,
            ...
            explanation = await explainUsingAWSBedrock(
                apiKey,
                sysPrompt,
                userPrompt,
                model,
...
async function explainUsingGPT(
    apiKey: string,
    sysPrompt: string,
    userPrompt: string,
    model: string,
...
async function explainUsingClaude(
    apiKey: string,
    sysPrompt: string,
    userPrompt: string,
    model: string,
...
async function explainUsingAWSBedrock(
    apiKey: string,
    sysPrompt: string,
    userPrompt: string,
    model: string,
...
export async function verifyHighlightedDafny(
    model: string,
    selectedText: string,
    filePath: string,
    outputChannel: vscode.OutputChannel,
    ...
    try {
        ...
        vscode.window.showErrorMessage(
            `Error verifying Dafny function: ${(error as Error).message}`,
        );
        console.error(error);
    }
}
